...
def get_config(case):
    if case == 'train_decoding': 
        ...
        parser.add_argument('-train_input', '--train_input', help='add noise' ,required=True)
        parser.add_argument('-cuda', '--cuda', help='specify cuda device name, e.g. cuda:0, cuda:1, etc', default = 'cuda:0')
        # Optional name to route eval outputs (best/last runs, ablation runs, etc).
        # eval_decoding.py also reads this; we include it here so argparse.parse_args() doesn't fail.
        parser.add_argument('--result_name', type=str, default=None, required=False)
        args = vars(parser.parse_args())

...
class SpectralBandAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Preserve the input scale: uniform attention must be an identity gate,
        # not an unintended 1 / n_bands attenuation. Learned gates redistribute
        # emphasis around a mean multiplier of one.
        x_weighted = x_bands * (attn_weights * self.n_bands)
        return x_weighted.reshape(B, L, -1)  # (B, L, 840)


class MultiScaleConv1D(nn.Module):
    """Parallel 1D convolutions with kernel sizes 3,5,7 to capture
    multi-scale local temporal EEG patterns."""
    def __init__(self, in_channels, out_channels, dropout=0.2, kernels=(3, 5, 7)):
        super().__init__()
...
class CrossAttentionBridge(nn.Module):
    """Word-aligned residual cross-attention over the EEG encoder sequence."""
    def __init__(self, d_encoder, d_decoder, n_heads=8, max_len=56, dropout=0.2):
        super().__init__()
        # Project each word-aligned encoder state into decoder space. Using the
        # projected EEG states as queries retains local alignment; the previous
        # fixed learned queries could ignore the sample and collapse to a
        # language prior.
        self.query_proj = nn.Linear(d_encoder, d_decoder)
    ...
    def forward(self, encoder_out, key_padding_mask=None):
        ...
        queries = self.query_proj(encoder_out)
...
class EEGConformer(nn.Module):
    ...
    def addin_forward(self, input_embeddings_batch, input_masks_invert):
        ...
        valid = (~input_masks_invert).unsqueeze(-1).to(input_embeddings_batch.dtype)
        # Normalize raw EEG
        x = self.input_norm(input_embeddings_batch) * valid

        # Input noise augmentation for regularization
        if self.training:
            x = (x + torch.randn_like(x) * 0.02) * valid

        # Spectral Band Attention (skip if ablated, but keep uniform support)
        if self.ablate_sba == 'uniform' or not self.ablate_sba:
            x = self.band_attention(x) * valid
        # else: pass through unchanged (raw ablate_sba fallback)

        # Multi-scale convolution (skip if ablated)
        if not self.ablate_multiscale:
            x = self.multi_scale_conv(x) * valid
        else:
            x = self.linear_conv(x) * valid

        # BiLSTM
        lengths = (~input_masks_invert).sum(dim=1).to("cpu")
